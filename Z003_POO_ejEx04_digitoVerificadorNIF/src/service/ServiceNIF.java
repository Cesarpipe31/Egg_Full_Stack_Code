...
public class ServiceNIF {
    ...
    public NIF crearNIF(){        
        n1.setDNI(crearDNI());
        int posLetra = (int)n1.getDNI() % 23;
        String listaCaracteres = "TRWAGMYFPDXBNJZSQVHLCKE";
        char letra = listaCaracteres.charAt(posLetra); 
        n1.setLetra(letra);
        return n1;
    }
    
    public long crearDNI() {
        long codigo = 0;
        int unidad = 1;
        System.out.println("Generando DNI aleatorio.......");
        for (int i = 0; i < 8; i++) {
    ...
    public void mostrar(){
        System.out.println("Su identificación es: " + n1.getDNI() + " - " + n1.getLetra());
    }
}
